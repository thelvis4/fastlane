module FastlaneCore
  ...
  class Project
    ...
    def make_xcodebuild_parameters(workspace: nil, scheme: nil, project: nil)
      proj = []
      proj << "-workspace #{workspace.shellescape}" if workspace
      proj << "-scheme #{scheme.shellescape}" if scheme
      proj << "-project #{project.shellescape}" if project

      proj
    end

    def xcodebuild_parameters
      make_xcodebuild_parameters(
    ...
    # Return the path to .xcodeproj file
    def project_path
      # We can find the project given the workspace and the scheme
      if workspace?
    ...
    # Returns the options to be passed to `xcodebuild -list`
    def xcodebuild_list_options
      project = project_path
      if project.nil?
        # If we cannot find the project (improbable scenario, but anyway),
        # we pass the workspace
        make_xcodebuild_parameters(workspace: options[:workspace])
      else
        make_xcodebuild_parameters(project: project_path)
      end
    end

    def build_xcodebuild_list_command
      options = xcodebuild_list_options
